    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 11) {
            ...
            HStack(spacing: 10) {
                ...
                SetupField(
                    placeholder: "you@example.com", text: $state.setupEmail, field: .email, focused: $focusedField
                )
            }
        ...
        VStack(alignment: .leading, spacing: 11) {
            ...
            if !state.status.isClean {
                RewindCard(
                    ...
                    detail: "\(state.status.changes.count) file\(state.status.changes.count == 1 ? "" : "s") "
                        + "would go back to how they were at your last save."
...
struct ConfirmDiscardView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 11) {
            ...
            Text(
                "These \(state.status.changes.count) file\(state.status.changes.count == 1 ? "" : "s") will go "
                    + "back to how they were at your last save."
            )
                .font(Theme.body(12))
                .foregroundStyle(Theme.textDim)
                .fixedSize(horizontal: false, vertical: true)
...
struct ConflictsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ActionButton(
                    title: state.allConflictsResolved
                        ? "All done — finish up"
                        : "\(resolved) of \(state.conflicts.count) sorted",
